...
7. **`A16`+ -- beating `A11`'s rating-74 ceiling** (`ideas/A16 ai agent strength
   improvement candidates/about.md`, captured 2026-09-09 right after `A14` registered
   at 62, well below `A11`). Not yet scheduled or designed in detail -- a ranked set of
   candidate directions, not a committed plan. Top-ranked candidate: genuine
   multi-round self-play bootstrapping (every trained net so far, `A11`'s and `A14`'s,
   was trained on exactly one round of imitation from an existing agent -- no agent
   has ever been retrained on data generated by itself, the one ingredient
   AlphaZero-class engines actually derive most of their strength from). Second:
   pthread-based root parallelization (flagged repeatedly, never implemented -- likely
