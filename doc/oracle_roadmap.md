...
7. **`A16`+ -- beating `A11`'s rating-74 ceiling** (`ideas/A16 ai agent strength
   improvement candidates/about.md`). Round 1 of the top-ranked candidate (genuine
   multi-round self-play bootstrapping) is ✅ **done, 2026-09-22/23**: `A14` retrained
   on a corpus adding its own self-play data (pooled with the original) beat the
   round-0 baseline by +2.25pp vs `A11` (n=8220, 95% CI excludes 0), promoted in place
   as "AlphaOracle Prime Plus II" -- rating now **~76**, a new roster ceiling above
   `A11`'s own 74. A same-day follow-up also confirmed PUCT selection isn't rescued
   by the stronger net either (-7.71pp vs plain UCT, same net) -- that question is
   closed. Round 2 is planned but **not started** (`about.md`'s "Session 4 plan"
   section has the full recipe and two things round 1 resolved: keep the corpus
   mixed across all rounds rather than going pure self-play, and don't re-test PUCT
   selection) -- paused 2026-09-23 as Jonathan's attention shifts to SDL3 GUI work
   below. pthread-based root parallelization (Candidate 2) lost its original
   "prerequisite" urgency once `use_puct=false` shipped at ~536ms/decision, so it's
   no longer blocking anything here. See the `about.md` for the full ranking,
   reasoning, and two more speculative candidates.
