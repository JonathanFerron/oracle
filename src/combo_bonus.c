...
void count_by_order(CombatCard *cards, int n, int *counts) {
    for (int i = 0; i < n; i++) {
        ChampionOrder order = get_order_from_species(cards[i].species);
...
int third_matches_order_of_species_pair(CombatCard *cards, int n) {
    ...
    for (int i = 0; i < n; i++) {
        if (cards[i].species == paired) {
            paired_order = get_order_from_species(cards[i].species);
        } else {
            singleton = &cards[i];
        }
    }
    ...
    ChampionOrder singleton_order = get_order_from_species(singleton->species);
    return (singleton_order == paired_order && 
...
int third_matches_color_of_order_pair(CombatCard *cards, int n) {
    ...
    // Find which order appears twice
    int order_counts[ORDER_COUNT] = {0};
    ...
    // Find the two cards with matching order and the singleton
    CombatCard *order_match_cards[2] = {NULL, NULL};
    CombatCard *singleton = NULL;
    int match_count = 0;
    
    for (int i = 0; i < n; i++) {
        ChampionOrder card_order = get_order_from_species(cards[i].species);
        if (card_order == paired_order) {
    ...
    // Check if singleton matches color of either order-matched card
