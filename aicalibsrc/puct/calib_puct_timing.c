// calib_puct_timing.c
// A14 AlphaOracle Prime Plus I per-decision timing harness -- mirrors
// aicalibsrc/ismctsnn/calib_ismctsnn_timing.c's exact structure (timing
// lives only in harnesses like this one, never in src/). Answers "does
// this agent's per-decision cost stay in the same ballpark as A11's own
// measured 439ms at -O2" *before* committing to a large-n Stage 5 run, not
// after -- see doc/ai_agents.md's A14 section's own expectation (~8% over
// A11, one shared forward pass now yielding both value and policy).
//
// Unlike calib_ismctsnn_timing.c, there is no trust dial to sweep here --
// this agent's leaf evaluation always uses the two-head net's one shared
// forward pass whenever it's active (see ai_strat_puct_search.c's
// puct_leaf_value()), so there is only one cost to measure, not a blend
// curve.
//
// Usage: calib_puct_timing <weights_path> <limit_iterations> <numgames> <seed>
...
{ double start = now_seconds();
  puct_attack_strategy(gstate, ctx);
...
{ double start = now_seconds();
  puct_defense_strategy(gstate, ctx);
  record_sample(gstate, now_seconds() - start);
...
{ StrategySet* strategies = create_strategy_set();
  set_player_strategy_by_type(strategies, PLAYER_A, AI_STRATEGY_ISMCTS_PUCT);
  set_player_strategy_by_type(strategies, PLAYER_B, AI_STRATEGY_ISMCTS_PUCT);
...
{ if(argc != 5)
  { fprintf(stderr,
            "Usage: %s <weights_path> <limit_iterations> <numgames> <seed>\n", argv[0]);
    return EXIT_FAILURE;
  }
  ...
  { fprintf(stderr, "calib_puct_timing: failed to load weights from '%s'\n", weights_path);
    return EXIT_FAILURE;
  }

  ISMCTSParams params = puct_get_default_ismcts_params();
  ...
  printf("weights=%s limit_iterations=%u numgames=%u seed=%lu total_decisions=%u\n",
         weights_path, limit_iterations, numgames, seed, g_sample_count);
  report_all_splits();
