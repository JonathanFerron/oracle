...
- **AI strategies beyond Random and Value Based are stubs.** `A1` Value Based ("The Apprentice", `src/ai_strat/ai_strat_valuebased.c`) was implemented 2026-08-21 — see `doc/changelog.md` for its scoring model and measured strength vs Random. CLI/config menus still list Combo Threshold/Balanced/Heuristic/Tactical/Hybrid/Monte Carlo/IS-MCTS as "not yet implemented" and fall back to Random; that "not yet implemented" label is now driven by `ai_strategy_is_implemented()` (`src/ai_strat/ai_strategy.c`)'s registry rather than hardcoded per-strategy, so a new agent needs only one line added to that registry to go live everywhere (CLI, TUI, `stda.auto`). Current active work is `A2` Combo Threshold, then `A3` Borealis in order — see `doc/oracle_todo.md`; TUI Milestones 1 and 2 are both done (`doc/changelog.md`). One `ideas/A#` folder per agent (`A1`-`A11`), numbered to match that agent's `AIStrategyType` enum ordinal (now declared in `src/core/game_types.h`, not `ui/shared/player_config.h`); general info and calibration tooling live in `ideas/G1`/`ideas/G2` instead of occupying an A-line slot — see `ideas/G1 AI agent general info/oracle_ai_agent_names.md` for the canonical roster and CLI shorthands (one per agent — no aliases).
