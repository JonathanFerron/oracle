...
- **AI strategies beyond Random are stubs.** CLI/config menus list Balanced/Heuristic/Hybrid/Monte Carlo/IS-MCTS as "not yet implemented" and fall back to Random. Current active work is the first non-dumb AI strategy (`ideas/A1 ai agent value based/`, then `A2`-`A4` in order — see `doc/oracle_todo.md`); TUI Milestones 1 and 2 are both done (`doc/changelog.md`).
