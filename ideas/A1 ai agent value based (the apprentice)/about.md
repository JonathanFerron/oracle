...
| Shorthand | `value` |
| Est. Borealis rating | 15 — design intent, unmeasured |
| Source file | `src/ai_strat/ai_strat_valuebased.c` (not yet written) |
...
- Touching the combo bonus in any way — the omission *is* the agent. It "knows one thing
  (efficiency) and nothing else" (roster naming rationale, #2).
- Subset/combination enumeration of any kind (that's `A3` Borealis).
- A pass option in the attack phase — ratio scoring has no natural zero to compare against;
  this agent always commits at least its top card.
