# Oracle TCG: Draft Formats Specification

## Overview

This document describes three advanced draft formats for Oracle TCG. Each format provides an alternative to Random, Monochrome, or Custom deck construction, offering varying levels of strategic depth, time commitment, and player interaction.

All three draft formats use modified combo bonuses that position deck strength between Random decks (weakest) and Custom/Monochrome decks (strongest).
...
## Combo Bonuses (All Draft Formats)
...
- **Order D (Eternal Light)**: Dragon, Cyclops, Fairy
- **Order E (Moonlight)**: Aven, Koatl, Lycan
...
**Expected Result**: Decks stronger than Random, weaker than Custom (unless extremely lucky), and comparable to Monochrome if player is experienced with drafting.
...
- **Players**: 2
- **Time**: 10-15 minutes
- **Rounds**: 14 drafting rounds
- **Key Mechanic**: Asymmetric pick pattern with face-up cards
...
4. **Second picker**: Selects 3 cards from the 5 remaining
5. **First picker**: Automatically receives the final 2 cards
6. All selected cards go face-down into each player's drafted pool
...
- Set aside discarded cards (not used in game)
...
- **Players**: 2
- **Time**: 15-18 minutes
- **Rounds**: 6 pile selections with pick-and-pass
- **Key Mechanic**: Pile selection from shared pool, then Rochester-style passing
...
4. Arrange all 12 piles in the center of the table (accessible to both players)

### Draft Process (6 Rounds Total)
...
4. Both players pass remaining 7 cards face-down to opponent
5. Both players look at the 7 received cards
6. Both players pick 1 card from the 7
7. Pass remaining 6 cards
...
- Round 4: Player B picks first
- Round 5: Player A picks first
- Round 6: Player B picks first

*Each player gets first pick 3 times for balance.*

**Result**: Each player drafts 48 cards (6 piles × 8 cards)
...
- Each player builds a 40-card deck from their remaining 40 cards
- Set aside discarded cards as sideboard
...
**Note**: This format is statistically equivalent to dealing 6 piles to each player separately, but the shared pool creates more engaging gameplay and player interaction.
...
- **Players**: 2
- **Time**: 24-25 minutes
- **Rounds**: 17 drafting rounds
- **Key Mechanic**: Rochester-style draft with 1-2-3 pick pattern
...
- Both players simultaneously choose 1 card
- Place chosen card in your drafted pool
- **Discard the remaining 3 cards** (removed from game, not to discard pile)

**Result**: After 17 rounds, each player has drafted 51 cards (17 rounds × 3 picks)
...
- Round 8: You have 2 Centaurs already
  - Look for: 3rd Centaur (+12), or Hobbit/Faun (Order B, +9/+5)
- Round 15: Opponent has been taking Red champions
...
- Maintain balanced mana curve (you need to afford your champions)

### Advanced Concepts

**Signaling**: In early rounds, the cards you pass can "signal" which colors/species you're NOT focusing on. Experienced players can use this to communicate or deceive.

**Hate-Drafting**: Taking a card your opponent needs, even if it doesn't fit your deck. Most effective in picks 2-3 when you've seen what opponent might want.

**Pivot Strategy**: If your initial plan isn't working (not enough cards of your species), be willing to switch focus mid-draft.
...
typedef enum {
    ...
    COMBO_CLASS_MONOCHROME = 1,  // Mono + Custom (no color combos)
    COMBO_CLASS_DRAFT = 2        // Solomon, Draft 12×8, Draft 1-2-3
} ComboClass;

ComboClass get_combo_class(DeckType deck_type) {
    switch(deck_type) {
        case DECK_RANDOM: 
            return COMBO_CLASS_RANDOM;
        case DECK_MONOCHROME:
        case DECK_CUSTOM:
            return COMBO_CLASS_MONOCHROME;
        case DECK_SOLOMON:
        case DECK_DRAFT_12X8:
        case DECK_DRAFT_123:
            return COMBO_CLASS_DRAFT;
        default:
            return COMBO_CLASS_RANDOM;
    }
}
...
const int DRAFT_COMBO_SPECIES_2_ORDER_1 = 9;
const int DRAFT_COMBO_SPECIES_2_COLOR_1 = 8;
const int DRAFT_COMBO_SPECIES_2 = 7;
const int DRAFT_COMBO_ORDER_3 = 6;
const int DRAFT_COMBO_ORDER_2_COLOR_1 = 5;
const int DRAFT_COMBO_ORDER_2 = 4;
const int DRAFT_COMBO_COLOR_3 = 4;
const int DRAFT_COMBO_COLOR_2 = 3;
```
...
- Analyze drafted pool composition:
  ...
  - Order distribution (A through E)
  - Color distribution (Red, Orange, Indigo)
  - Average card power
...
- Analyze final deck composition:
  ...
  - Actual order combos included
  - Actual color combos included
...
- Track metrics:
  ...
  - Win rate by player position (first vs second)
  - Average damage per turn
  - Combo frequency during gameplay
  - Luna expenditure patterns
...
Since piles are face-down and random:
  ...
  - Can implement "human-like" preferences for engagement
...
For each card in hand:
  score = (standalone_value * weight_standalone) +
          (combo_potential * weight_combo) +
          (flexibility * weight_flexibility)

Weights adjust based on:
  - Pick number (1st through 8th)
  - Round number (1-6)
...
Middle rounds (7-12):
  weight_standalone = 0.3
  weight_combo = 0.5
  weight_flexibility = 0.2

Late rounds (13-17):
  weight_standalone = 0.2
  weight_combo = 0.6
  weight_flexibility = 0.2
```
...
- Creates exciting moments (split decisions)

---

## Tournament Structure Suggestions
...
- Tests adaptability and format mastery
...
- Draft 1-2-3: 17 rounds, 51 cards, Rochester-style (uses 2 decks of 120 cards each)
- Unified combo bonuses for all draft formats
- Positioned between Random and Monochrome/Custom in power level
...
- Each format offers unique strategic experience
...
4. Provide options for different time commitments
...
- All three draft formats should produce similar power levels
- Skill should matter more in drafting than in Random deck play
...
├─ 15 minutes or less → Solomon 7×7
├─ 15-20 minutes → Draft 12×8
└─ 20+ minutes → Draft 1-2-3

What's your experience level?
├─ New to drafting → Solomon 7×7
├─ Some draft experience → Draft 12×8
└─ Experienced drafter → Draft 1-2-3

What's your play style?
├─ Like seeing all information → Solomon 7×7
├─ Like balanced gameplay → Draft 12×8
└─ Like deep strategy → Draft 1-2-3

How many decks do you have?
├─ One 120-card deck → Solomon 7×7 or Draft 12×8
└─ Two 120-card decks → Draft 1-2-3
```
...
This document represents the complete specification for Oracle TCG's three draft formats as of the current version. All formats have been designed for 2 players with balanced gameplay, strategic depth, and appropriate time commitments for different play contexts.

For questions, clarifications, or proposed modifications, please refer to the official Oracle TCG documentation or contact the game designers.